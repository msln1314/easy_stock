...
from backend.services.trade.services.position_accounting_service import position_accounting_service
...
def _build_real_account_contract(
    *,
    ...
    # 持仓核算：计算实时盈亏和持仓结构
    realtime_pnl = position_accounting_service.calculate_realtime_pnl(
        positions=positions,
        total_asset=float(total_asset or 0.0),
        today_pnl_raw=float(broker_today_pnl_raw or 0.0),
        total_pnl_raw=float(total_pnl_raw or 0.0),
        initial_equity=float(initial_equity or 0.0),
    )
    position_structure = position_accounting_service.analyze_position_structure(
        positions=positions,
        total_asset=float(total_asset or 0.0),
    )
