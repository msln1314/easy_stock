...
class TestStockPickAPI:
    ...
    def test_list_endpoint_exists(self, session):
        """GET /api/v1/stock-pick 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/stock-pick")
        assert resp.status_code in [200, 401, 403]

    def test_create_endpoint_exists(self, session):
        """POST /api/v1/stock-pick 接口存在"""
        resp = session.post(
            f"{BASE_URL}/api/v1/stock-pick",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403]

    def test_get_endpoint_exists(self, session):
        """GET /api/v1/stock-pick/{id} 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/stock-pick/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_update_endpoint_exists(self, session):
        """PUT /api/v1/stock-pick/{id} 接口存在"""
        resp = session.put(
            f"{BASE_URL}/api/v1/stock-pick/1",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403, 404]

    def test_delete_endpoint_exists(self, session):
        """DELETE /api/v1/stock-pick/{id} 接口存在"""
        resp = session.delete(f"{BASE_URL}/api/v1/stock-pick/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_execute_endpoint_exists(self, session):
        """POST /api/v1/stock-pick/{id}/execute 接口存在"""
        resp = session.post(f"{BASE_URL}/api/v1/stock-pick/1/execute")
        assert resp.status_code in [200, 400, 401, 403, 404]

    def test_results_endpoint_exists(self, session):
        """GET /api/v1/stock-pick/{id}/results 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/stock-pick/1/results")
        assert resp.status_code in [200, 401, 403, 404]

    def test_history_endpoint_exists(self, session):
        """GET /api/v1/stock-pick/history 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/stock-pick/history")
        assert resp.status_code in [200, 401, 403]
