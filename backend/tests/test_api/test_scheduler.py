...
class TestSchedulerAPI:
    ...
    def test_list_endpoint_exists(self, session):
        """GET /api/scheduler 接口存在"""
        resp = session.get(f"{BASE_URL}/api/scheduler")
        assert resp.status_code in [200, 401, 403]

    def test_create_endpoint_exists(self, session):
        """POST /api/scheduler 接口存在"""
        resp = session.post(
    ...
    def test_get_endpoint_exists(self, session):
        """GET /api/scheduler/{id} 接口存在"""
        resp = session.get(f"{BASE_URL}/api/scheduler/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_update_endpoint_exists(self, session):
        """PUT /api/scheduler/{id} 接口存在"""
        resp = session.put(
            f"{BASE_URL}/api/scheduler/1",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403, 404]

    def test_delete_endpoint_exists(self, session):
        """DELETE /api/scheduler/{id} 接口存在"""
        resp = session.delete(f"{BASE_URL}/api/scheduler/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_run_endpoint_exists(self, session):
        """POST /api/scheduler/{id}/run 接口存在"""
        resp = session.post(f"{BASE_URL}/api/scheduler/1/run")
        assert resp.status_code in [200, 401, 403, 404]

    def test_pause_endpoint_exists(self, session):
        """POST /api/scheduler/{id}/pause 接口存在"""
        resp = session.post(f"{BASE_URL}/api/scheduler/1/pause")
        assert resp.status_code in [200, 401, 403, 404]

    def test_resume_endpoint_exists(self, session):
        """POST /api/scheduler/{id}/resume 接口存在"""
        resp = session.post(f"{BASE_URL}/api/scheduler/1/resume")
        assert resp.status_code in [200, 401, 403, 404]
