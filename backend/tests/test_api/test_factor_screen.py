...
class TestFactorScreenAPI:
    ...
    def test_list_endpoint_exists(self, session):
        """GET /api/v1/factor-screen 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/factor-screen")
        assert resp.status_code in [200, 401, 403]

    def test_create_endpoint_exists(self, session):
        """POST /api/v1/factor-screen 接口存在"""
        resp = session.post(
            f"{BASE_URL}/api/v1/factor-screen",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403]

    def test_get_endpoint_exists(self, session):
        """GET /api/v1/factor-screen/{id} 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/factor-screen/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_update_endpoint_exists(self, session):
        """PUT /api/v1/factor-screen/{id} 接口存在"""
        resp = session.put(
            f"{BASE_URL}/api/v1/factor-screen/1",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403, 404]

    def test_delete_endpoint_exists(self, session):
        """DELETE /api/v1/factor-screen/{id} 接口存在"""
        resp = session.delete(f"{BASE_URL}/api/v1/factor-screen/1")
        assert resp.status_code in [200, 401, 403, 404]

    def test_screen_endpoint_exists(self, session):
        """POST /api/v1/factor-screen/screen 接口存在"""
        resp = session.post(
            f"{BASE_URL}/api/v1/factor-screen/screen",
            json={}
        )
        assert resp.status_code in [200, 400, 401, 403]

    def test_factors_endpoint_exists(self, session):
        """GET /api/v1/factor-screen/factors 接口存在"""
        resp = session.get(f"{BASE_URL}/api/v1/factor-screen/factors")
        assert resp.status_code in [200, 401, 403]
    ...
    def test_list_requires_auth(self, helper):
        """获取因子筛选列表需要登录"""
        resp = helper.test_public_endpoint("/api/v1/factor-screen")
        assert resp.status_code == 401

    def test_list_with_token(self, helper, admin_token):
        """有token可以获取因子筛选列表"""
        resp = helper.test_auth_endpoint("/api/v1/factor-screen", admin_token)
    ...
    def test_get_requires_auth(self, helper):
        """获取单个因子筛选需要登录"""
        resp = helper.test_public_endpoint("/api/v1/factor-screen/1")
        assert resp.status_code == 401

    def test_update_requires_auth(self, helper):
        """更新因子筛选需要登录"""
        resp = helper.test_public_endpoint(
            "/api/v1/factor-screen/1",
            method="PUT",
            data={}
        )
        assert resp.status_code == 401
    ...
    def test_delete_requires_auth(self, helper):
        """删除因子筛选需要登录"""
        resp = helper.test_public_endpoint("/api/v1/factor-screen/1", method="DELETE")
        assert resp.status_code == 401

    def test_screen_requires_auth(self, helper):
        """执行因子筛选需要登录"""
        resp = helper.test_public_endpoint(
            "/api/v1/factor-screen/screen",
            method="POST",
            data={}
        )
        assert resp.status_code == 401
    ...
    def test_factors_requires_auth(self, helper):
        """获取可用因子列表需要登录"""
        resp = helper.test_public_endpoint("/api/v1/factor-screen/factors")
        assert resp.status_code == 401

    def test_factors_with_token(self, helper, admin_token):
        """有token可以获取可用因子列表"""
        resp = helper.test_auth_endpoint("/api/v1/factor-screen/factors", admin_token)
        assert resp.status_code == 200
        assert helper.validate_response_structure(resp)