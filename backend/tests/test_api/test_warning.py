...
class TestWarningAPI:
    ...
    def test_indicators_requires_login(self, helper):
        """获取指标列表需要登录"""
        resp = helper.test_public_endpoint("/api/v1/warning/indicators")
        assert resp.status_code == 401

    def test_conditions_requires_login(self, helper):
        """获取条件列表需要登录"""
        resp = helper.test_public_endpoint("/api/v1/warning/conditions")
        assert resp.status_code == 401

    def test_stocks_requires_login(self, helper):
        """获取股票列表需要登录"""
        resp = helper.test_public_endpoint("/api/v1/warning/stocks")
        assert resp.status_code == 401
